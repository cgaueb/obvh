...
void AgglomerativeTreeletOptimizer::Optimize(
    BVHTree* deviceTree,
    float * time_ms, float* sah,
    float ci, float ct)
{
    ...
    checkCudaError(cudaMalloc(&deviceSubtreeTrianglesCount,  (2 * numberOfTriangles - 1) * sizeof(int)));
    ...
    {
        checkCudaError(cudaMemset(deviceCounters, 0x00, (numberOfTriangles - 1) * sizeof(unsigned int)));
        ...
        if (treeletSize > MAX_TREELET_SIZE_DIST_SHARED_MEM)
        {
            elapsedTime += DeviceAgglomerativeTreeletOptimizer(numberOfTriangles, deviceTree,
                ...
                scheduleSize, deviceDistanceMatrix, numberOfElements, deviceNodesSah, 10.f, 20.f);
        }
        else
        {
            elapsedTime += DeviceAgglomerativeSmallTreeletOptimizer(numberOfTriangles, deviceTree,
                deviceCounters, deviceSubtreeTrianglesCount, treeletSize, gamma, deviceSchedule,
                scheduleSize, deviceNodesSah, ci, ct);
        }
    ...
    if (time_ms) *time_ms = elapsedTime;
    ...
    if (true)
    {
        ...
        std::cout << "\tSAH: " << hostTree->SAH(ci, ct) << std::endl;
