...
__device__ void findMinimum(int numberOfElements, int& index, float& distance)
{
    ...
    {
        int otherIndex = __shfl_down_sync(BVH_FULL_MASK, index, shflAmount);
        float otherArea = __shfl_down_sync(BVH_FULL_MASK, distance, shflAmount);
...
__device__ void findMinimumDistance(float* distancesMatrix, int lastRow, int& minIndex)
{
    ...
    minIndex = __shfl_sync(BVH_FULL_MASK, minIndex, 0);
}
    ...
    float* treeletLeavesAreas, float* bbMin, float* bbMax, float ci)
{
    ...
    // not going to work
    float3 bbMinA, bbMaxA, bbMinB, bbMaxB;
    float sah = __shfl_sync(BVH_FULL_MASK, threadSah, joinRow) + __shfl_sync(BVH_FULL_MASK, threadSah, joinCol);
    int leftIndex = __shfl_sync(BVH_FULL_MASK, threadNode, joinRow);
    int rightIndex = __shfl_sync(BVH_FULL_MASK, threadNode, joinCol);
    ...
    // both blocks
    int lastIndex = __shfl_sync(BVH_FULL_MASK, threadNode, lastRow);
    float sahLast = __shfl_sync(BVH_FULL_MASK, threadSah, lastRow);
...
__device__ void updateTreelet_NoOp(int treeletSize, BVHTree* tree, int threadNode,
    int* treeletInternalNodes, int* treeletLeaves, float* treeletLeavesAreas,
    float* nodesSah, float* bbMin, float* bbMax)
{
    if (THREAD_WARP_INDEX >= 1 && THREAD_WARP_INDEX < treeletSize)
    ...
    float* nodesSah, float ci, float ct)
{
    ...
    while (__ballot_sync(BVH_FULL_MASK, currentNodeIndex >= 0) != 0)
    {
        __threadfence();
        ...
        // triangles
        unsigned int vote = __ballot_sync(BVH_FULL_MASK, triangleCount >= gamma);
        ...
        while (vote != 0)
        {
            ...
            // variable
            int treeletRootIndex = __shfl_sync(BVH_FULL_MASK, currentNodeIndex, rootThreadIndex);
            ...
            formTreelet(treeletRootIndex, numberOfTriangles, tree, treeletSize,
                ...
                WARP_ARRAY(treeletLeavesAreas, treeletSize), BVH_FULL_MASK);
