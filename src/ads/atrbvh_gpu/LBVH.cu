...
__global__ void generateMortonCodesTSKernel(unsigned int numberOfElements, const BoxElements* elements,
        ...
        unsigned int* routingIndices)
{
    ...
    float3 sceneBboxMin = elements->boundingBoxMin;
    float3 sceneBboxMax = elements->boundingBoxMax;

    // Calculate bounding box centroid
    float4 centroid4 = elements->centroids[globalId];
    float3 centroid{};
    centroid.x = centroid4.x;
    centroid.y = centroid4.y;
    centroid.z = centroid4.z;
...
__global__ void generateMortonCodes64TSKernel(unsigned int numberOfElements, const BoxElements* elements,
        ...
        unsigned int* routingIndices)
{
        ...
        int index1, int index2, unsigned int key1)
    {
        ...
        if (index2 < 0 || index2 >= numberOfElements) return -1;
...
#define divCeil(l, k) l / k + !!(l % k);
    ...
    int leftIndex = splitPosition;
    int rightIndex = splitPosition + 1;
    ...
    // interior node does not point to anything
...
__global__ void calculateNodeBoundingBoxesKernelBB(unsigned int numberOfElements,
    const BoxElements* elements, BVHTree* tree, unsigned int* counters)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    int index = i + numberOfElements - 1;
    ...
    float4 cacheMin = elements->elem_boundingBoxMin[dataIndex];
    float4 cacheMax = elements->elem_boundingBoxMax[dataIndex];
    ...
    while (current != -1)
    {
        ...
        calculateNodeBoundingBox(&cacheMin, &cacheMax, &cacheMin, &cacheMax, &childBbMin, &childBbMax);
        sharedBbMin[threadIdx.x] = cacheMin;
        sharedBbMax[threadIdx.x] = cacheMax;
        ...
        const Scene* scene, BVHTree* tree, unsigned int* counters)
{
    ...
    int index = i + numberOfTriangles - 1; // first n - 1 are interior
    int dataIndex = tree->DataIndex(index); // get prim idx
    ...
    while (current != -1)
    {
        ...
        // children of this node
        // example :
        // right gets here first and dies
        // left gets here second and discovers about the right node thread idx
        // !!exchange atomic in conjuction with binary branching ensures that both children nodes!!
        // !!are updated properly before read!!
        unsigned int childThreadId = atomicExch(&counters[current], i);
        ...
        // example cont. :
        // if right node was on the same block as left (the currently alive one)
        // find his bbox in shared memory
        if (childThreadId >= firstThreadInBlock && childThreadId <= lastThreadInBlock)
        {
            // If both child nodes were processed by the same block, we can reuse the values
            // cached in shared memory
...
float DeviceGenerateMortonCodes(unsigned int numberOfTriangles, const BoxElements* scene,
        float4* boundingBoxesMin, float4* boundingBoxesMax, unsigned int* mortonCodes, 
        unsigned int* routingIndices)
{
    dim3 blockSize(256, 1, 1);
        ...
        unsigned int* sortIndices, BVHTree* tree)
{
    ...
    return TimeKernelExecution([&]()
    {
        buildTreeKernel<unsigned int><<<gridSize, blockSize>>>(numberOfTriangles, sortedKeys, sortIndices, tree);
    });
}
...
float DeviceCalculateNodeBoundingBoxes(unsigned int numberOfElements, const BoxElements* elements,
    BVHTree* tree, unsigned int* counters)
{
    dim3 blockSize(128, 1, 1);
    dim3 gridSize((numberOfElements + (blockSize.x - 1)) / blockSize.x, 1, 1);
    ...
    cudaFuncSetCacheConfig(calculateNodeBoundingBoxesKernelBB, cudaFuncCachePreferShared);

    return TimeKernelExecution([&]()
        {
            calculateNodeBoundingBoxesKernelBB<<<gridSize, blockSize, bboxCacheSize>>>(
                numberOfElements, elements, tree, counters);
        });
}
