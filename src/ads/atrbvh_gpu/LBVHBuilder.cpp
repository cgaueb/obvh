...
LBVHBuilder::LBVHBuilder(bool use64BitMortonCodes) :
    use64BitMortonCodes(use64BitMortonCodes)
{ /* Empty */ }

LBVHBuilder::~LBVHBuilder()
{ /* Empty */ }

BVHTree* LBVHBuilder::BuildTree(const SceneWrapper* sceneWrapper, float ci, float ct, float* sah, float* time_ms)
{
    BVHTree* tree = nullptr;
    if (use64BitMortonCodes)
    {
        tree = BuildTreeNoTriangleSplitting<unsigned long long int>(sceneWrapper, ci, ct, sah, time_ms);
    }
    else
    {
        tree = BuildTreeNoTriangleSplitting<unsigned int>(sceneWrapper, ci, ct, sah, time_ms);
    }
...
template <typename T> BVHTree* LBVHBuilder::BuildTreeNoTriangleSplitting(
    const SceneWrapper* sceneWrapper,
    float ci, float ct,
    float* sah, float* time_ms) const
{
    CudaTimer timer;
    timer.Start();
    ...
    checkCudaError(cudaMemset(deviceCounters, 0xFFFFFFFF,
    ...
    float treeSAH = hostTree->SAH(ci, ct);
    
    float total_time = timer.ElapsedTime();
    if (sah) *sah = treeSAH;
    if (time_ms) *time_ms = timeMortonCodes + timeSort + timeBuildTree + timeBoundingBoxes;

    if (true)
    {
        std::cout << "\tLBVH: Scene area: " << hostTree->Area(hostTree->RootIndex()) << std::endl;
        std::cout << "\tBuild time: " << total_time << " ms" << std::endl;
        std::cout << "\tSAH: " << treeSAH << std::endl;
