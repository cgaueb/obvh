...
#define BVH_FULL_MASK 0xffffffff
...
do { \
    int _sync = 0; \
    __shfl_sync(BVH_FULL_MASK, _sync, 0); \
} while (0)
#else
#define WARP_SYNC \
__syncthreads();
...
#define SHFL_FLOAT3(destination, source, index) \
    (destination).x = __shfl_sync(BVH_FULL_MASK, (source)[0], (index)); \
    (destination).y = __shfl_sync(BVH_FULL_MASK, (source)[1], (index)); \
    (destination).z = __shfl_sync(BVH_FULL_MASK, (source)[2], (index));
        ...
        unsigned long long int value)
{
    ...
    expanded &= 0x1fffffu;
    expanded = (expanded | expanded << 32) & 0x1f00000000ffffu;
    expanded = (expanded | expanded << 16) & 0x1f0000ff0000ffu;
    expanded = (expanded | expanded << 8) & 0x100f00f00f00f00fu;
    expanded = (expanded | expanded << 4) & 0x10c30c30c30c30c3u;
    expanded = (expanded | expanded << 2) & 0x1249249249249249u;
...
__forceinline__ __device__ unsigned long long int calculateMortonCode64(float3 point)
{
    // Discretize the unit cube into a 21 bit integer
    //unsigned long long int discretized[3];
    ulonglong4 discretized;
    discretized.x = (unsigned long long int)min(max(point.x * 2097152.0f, 0.0f), 2097151.0f);
    discretized.y = (unsigned long long int)min(max(point.y * 2097152.0f, 0.0f), 2097151.0f);
    discretized.z = (unsigned long long int)min(max(point.z * 2097152.0f, 0.0f), 2097151.0f);

    discretized.x = expandBits64(discretized.x);
    discretized.y = expandBits64(discretized.y);
    discretized.z = expandBits64(discretized.z);
    unsigned long long int ret = 0;
    //ret |= discretized[0] | discretized[1] << 1 | discretized[2] << 2;
